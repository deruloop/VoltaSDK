...
| `mentions` | `path` (required), `of` (required, an array path) | The text at `path` names at least one element of the array at `of`, by normalized containment or any content word of the element (four letters or more, function words excluded). A note that says "add berries" when the additions include "a handful of berries". | `.mentions(path:of:)` |
