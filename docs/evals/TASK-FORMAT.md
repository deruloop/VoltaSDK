...
| `expect-contains-any` | | For samples with `expect.containsAny`: at each path, at least one listed element is present, same normalization as `expect-contains`. For an answer that may pick any of several right things. Other samples: ignored. | `.expectContainsAny()` |
...
| `mentions` | `path` (required), `of` (required, an array path), `all` (bool, default false) | The text at `path` names at least one element of the array at `of` (every element with `all`, for a list that claims to be what the text names), by normalized containment or any content word of the element (four letters or more, function words excluded). A note that says "add berries" when the additions include "a handful of berries". | `.mentions(path:of:)` |
...
| `expect.containsAny` | no | array path → candidates, one of which must appear. Read by `expect-contains-any`. |
