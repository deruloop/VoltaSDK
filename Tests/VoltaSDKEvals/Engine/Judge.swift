...
struct JudgeConfiguration: Sendable {
    ...
    static func fromEnvironment(_ environment: [String: String] = ProcessInfo.processInfo.environment) -> JudgeConfiguration? {
    ...
    func mayJudge(_ tier: EvalTier) -> Bool {
        tier.vendor != vendor
    }
    ...
    func makeEvaluator(for task: EvalTask, tier: EvalTier) -> (any EvaluatorProtocol<FrameworkSample, ModelSubject<EvalOutcome>>)? {
        guard let spec = task.judge, !spec.dimensions.isEmpty, mayJudge(tier) else { return nil }
    ...
    @available(iOS 27.0, macOS 27.0, *)
    static func scale(for dimension: JudgeSpec.Dimension) -> ScoringScale {
...
struct HumanRatings: Codable {
    var task: String
    var ratings: [String: [String: Double]]

    static func load(for taskID: String, from path: String) -> HumanRatings? {
...
struct JudgeAgreement: Codable {
    ...
    static func measure(judge: [String: [String: Double]], human: HumanRatings) -> JudgeAgreement? {
